...
对于 $n$ 为偶数，虽然貌似也可以直接扫描线，但是困难。

考虑预处理前缀和后缀的中位数。

考虑每个元素对左半部分中位数 $p$ 的贡献。

发现对于 $q>p$ 有贡献，当且仅当 $[1,q]$ 为奇数且中位数等于 $a_p$。

对于 $q<p$ 有贡献，当且仅当 $[1,q]$ 为偶数且 $a_p$ 在两个中位数之间，注意这里并不意味着 $a_p$ 是两个中位数之一。

题解大概把它当成一个二维数点问题，实际上可以在扫描的时候直接算出来，直接做到线性。
...
「让所有路径都能在有限步走到所选点集」其实等价于「不存在不经过所选点集的无穷长路径」。

因此相当于选出一个点集割掉，使剩下的连通块至多只有一个叶子。

树形 DP 即可（其实也可以贪心）

### T2

注意到如果每个点的两条出边指向不同点，则答案为 $2^{n}-1$。

如果一个点的两条出边指向相同点，我们发现它对应的串是一个 border 为 1 的相同串。

所以我们找到所有连续段即可。

### T3

考虑找到 $m$ 的质因分解，直接矩阵快速幂可以做到 $O(q2^{3\omega(m)}\log n)$。

注意到如果两个质因子次数相同，则可以认为他们是等价的！我们可以只记录每个等价类包含多少个质因子即可，然后就做完了！！！

质因分解我<ruby>写<rt>抄</rt></ruby>了 MillerRabin + PollardRho，但是其实有简单方法。

其实我们只关心质因子次数集合。

不妨筛出 $O(m^{1/3})$ 的质因子，对于剩下部分只有三种可能：$m=p,m=p^2,m=p_1p_2$，用一个 MillerRabin 判断一下即可。
...
其实 meet in the middle 做完了。

记左边为 $la\le lb\le lc$，右边为 $ra\ge rb\ge rc$，由于两边独立，所以答案放 $R\{la+ra,lb+rb,lc+rc\}$ 是最优的，其中 $R$ 表示<ruby>极差<rt>Range</rt></ruby>。

考虑分 $3! =6$ 种情况讨论，比如 $la+ra\le lb+rb\le lc+rc$。

此时我们要求 $la-lb\le rb-ra,lb-lc\le rc-rb$，最小化 $lc-la+rc-ra$。
